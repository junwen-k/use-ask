...
describe('createCallStore', () => {
  ...
  describe('when using the hook', () => {
    it('should return the current call stack', () => {
    ...
    it('should update when calls are added to the store', () => {
      const [store, useCallStore] = createCallStore();
      const { result } = renderHook(() => useCallStore());
    ...
    it('should update when multiple calls are added', () => {
      const [store, useCallStore] = createCallStore();
      const { result } = renderHook(() => useCallStore());
    ...
    it('should handle per-call unmounting delay', async () => {
      const [store, useCallStore] = createCallStore();
      const { result } = renderHook(() => useCallStore());

      let callPromise: Promise<unknown>;
      act(() => {
        callPromise = store.call('test-payload', { unmountingDelay: 50 });
      });
