...
describe('createSingletonCallStore', () => {
  ...
  describe('when using the hook', () => {
    ...
    it('should return the current call when it exists', () => {
      ...
      expect(result.current?.promise).toBe(callPromise);
      expect(result.current?.payload).toBe('test-payload');
    });
    ...
    it('should update when the current call changes', () => {
      ...
      expect(result.current?.promise).toBe(firstPromise);
    ...
    it('should create a new call after resolution', async () => {
      ...
      expect(result.current?.promise).toBe(secondPromise);
  ...
  describe('when multiple hooks use the same store', () => {
    it('should keep all hooks in sync', () => {
      ...
      expect(result1.current?.promise).toBe(callPromise);
      expect(result2.current?.promise).toBe(callPromise);
    });
    ...
    it('should update all hooks when current call changes', () => {
      ...
      expect(result1.current?.promise).toBe(firstPromise);
      expect(result2.current?.promise).toBe(firstPromise);
