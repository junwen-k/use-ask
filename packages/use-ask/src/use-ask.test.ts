...
import { type UseAskReturn, useAsk } from './use-ask';
...
interface SafeAskTestContext<TData = unknown, TReason = unknown> {
  result: RenderHookResult<
    ReturnType<typeof useAsk<TData, TReason>>,
    unknown
  >['result'];
...
describe('safeAsk', () => {
  ...
  >('should resolve with data when promise resolves', async ({
    result,
    answerPromise,
  }) => {
