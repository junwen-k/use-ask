...
export const useAsk = <TData = unknown, TReason = unknown>(): UseAskReturn<
  TData,
  TReason
> => {
  const [{ resolve, reject, safe }, setState] = useState<
    UseAskState<TData, TReason>
  >({});

  const start = useCallback(<Safe extends boolean>(_safe: Safe) => {
    const {
      promise,
      resolve: _resolve,
      reject: _reject,
    } = Promise.withResolvers<
      Safe extends true ? UseAskResult<TData, TReason> : TData
    >();
