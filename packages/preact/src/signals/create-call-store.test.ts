...
describe('createCallStore (signals)', () => {
  ...
  describe('when using the hook', () => {
    ...
    it('should return the call stack when calls exist', () => {
      ...
      expect(result.current.value[0].payload).toBe('test-payload');
      expect(result.current.value[0].promise).toBeInstanceOf(Promise);
    });
    ...
    it('should update when new calls are added', () => {
      ...
      expect(result.current.value).toHaveLength(1);
      expect(result.current.value[0].payload).toBe('first-payload');
      ...
      expect(result.current.value[0].payload).toBe('first-payload');
      expect(result.current.value[1].payload).toBe('second-payload');
    });
    ...
    it('should update when call payloads are updated', () => {
      ...
      expect(result.current.value).toHaveLength(1);
      expect(result.current.value[0].payload).toBe('updated-payload');
    });
    ...
    it('should handle calls with unmounting delay', async () => {
      ...
      expect(result.current.value).toHaveLength(1);
      expect(result.current.value[0].pending).toBe(false);
  ...
  describe('when multiple hooks use the same store', () => {
    it('should keep all hooks in sync', () => {
      ...
      expect(result1.current.value).toHaveLength(1);
      expect(result2.current.value).toHaveLength(1);
      expect(result1.current.value[0].payload).toBe('test-payload');
      expect(result2.current.value[0].payload).toBe('test-payload');
    });
    ...
    it('should update all hooks when store changes', () => {
      ...
      expect(result1.current.value).toHaveLength(2);
      expect(result2.current.value).toHaveLength(2);
    });
  });
});
